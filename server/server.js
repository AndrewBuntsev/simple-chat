...
app.get('/api/anonymousLogin', function(req, res) {
  console.log('Anonymous login tried: ' + req.query.name);
  if (!req.query.name) {
    res.json({ err: 'Name must be not empty' });
    res.end();
    return;
  }

  let isNameFree = clients.find(c => c == req.query.name) == null;
  if (!isNameFree) {
    res.json({
      err: 'Name ' + req.query.name + ' is already taken. Choose another one'
    });
    res.end();
    return;
  }

  clients.push(req.query.name);

  res.json({
    success: 'Name ' + req.query.name + ' is approved'
  });
  res.end();
});
